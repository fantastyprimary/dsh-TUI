...
export async function apply(ctx, config) {
    ...
    const { agent, handle, agentPreset, smart, route: createdRoute } = await resolveAgent(ctx, config.sessionId, configuredRoute, startupRoute, meta, config.preset, config.smart ?? readSmartDefault() ?? false);
 ...
 */
async function resolveAgent(ctx, requestedSessionId, configuredRoute, startupRoute, meta, configuredPreset, startupSmart = false) {
