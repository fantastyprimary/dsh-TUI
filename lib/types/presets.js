 ...
 */
export async function composePreset(ctx, requested, smart = false, forceSmart = false) {
    if (smart && forceSmart)
        throw new Error('Smart and ForceSmart are mutually exclusive');
    const presets = rosterOf(ctx);
    if (presets === undefined) {
        if (smart)
            return { setup: agentCtx => mountSmartEnhancement(ctx, agentCtx) };
        if (forceSmart)
            return { setup: agentCtx => mountForceSmartEnhancement(ctx, agentCtx) };
        return {};
    }
    let resolvedId;
    try {
    ...
    return {
        ...
        setup: async (agentCtx) => {
            ...
            if (smart)
                await mountSmartEnhancement(ctx, agentCtx, resolvedId);
            else if (forceSmart)
                await mountForceSmartEnhancement(ctx, agentCtx, resolvedId);
        },
    };
}
/**
