...
function bootstrapEntry() {
    ...
    if (found === undefined)
        throw new Error('dsh-tui ForceSmart bootstrap asset is missing');
    return found;
}
async function loadBootstrap() {
    bootstrapModule ??= import(pathToFileURL(bootstrapEntry()).href);
    return await bootstrapModule;
}
async function mountMinimalTools(agentCtx, bashDescription, allowAdditions) {
...
export async function mountForceSmartEnhancement(hostCtx, agentCtx, basePreset) {
    ...
    const ownedTools = await mountMinimalTools(agentCtx, bootstrap.MINIMAL_BASH_SCHEMA.description, basePreset !== 'code');
