...
function stateFor(hostCtx) {
    const root = hostCtx.root;
    const existing = roots.get(root);
    if (existing !== undefined)
        return existing;
    const state = { mounted: new WeakMap() };
...
export function enhancementOf(hostCtx, agent) {
