...
import { forceSmartModeOf, readForceSmartDefault, resolvePersistedForceSmart, writeForceSmartSession, } from '../forceSmartPrefs.js';
...
export async function apply(ctx, config) {
    ...
    const { smart: startupSmart, forceSmart: startupForceSmart } = resolveEnhancementSelection(config.smart, config.forceSmart, readSmartDefault(), readForceSmartDefault());
    const { agent, handle, agentPreset, smart, forceSmart, route: createdRoute } = await resolveAgent(ctx, config.sessionId, configuredRoute, startupRoute, meta, config.preset, startupSmart, startupForceSmart);
    try {
        ...
        const attached = await attachSessionToWorkspace(ctx, agent.session.header.cwd ?? meta.cwd, agent.session.id);
        if (!attached) {
 ...
 */
async function resolveAgent(ctx, requestedSessionId, configuredRoute, startupRoute, meta, configuredPreset, startupSmart = false, startupForceSmart = false) {
    ...
    const composed = await composePreset(ctx, configuredPreset ?? readPresetPref(), startupSmart, startupForceSmart);
