...
export function findSmartHostRuntime(home = dshHome(), profile = resolveDshProfileName() ?? 'dsh-tui', override = process.env.DSH_SMART_RUNTIME_PATH?.trim()) {
    const candidates = [
...
export function verifySmartHostRuntime(location) {
    try {
        const metadata = JSON.parse(readFileSync(location.packageFile, 'utf8'));
...
export async function apply(ctx) {
    ...
    const smartTools = new Set(['dev_smart_status', ...SUPER_INJECTOR_TOOL_NAMES]);
    const smartContexts = new Set(['dsh-super-injector']);
    ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
        const assembled = await next();
        const agent = context.agent;
        if (agent !== undefined
            && (enhancementOf(ctx, agent) === 'smart' || smartModeOf(agent.session)))
            return assembled;
        return {
            ...assembled,
    ...
    if (location === undefined) {
        ctx.logger.warn(`dsh-tui Smart: optional host runtime v${SUPER_INJECTOR_VERSION} payload not found; `
    ...
    try {
        const injector = await import(pathToFileURL(location.entry).href);
        if (typeof injector.apply !== 'function')
            throw new Error('host bundle has no apply() export');
        ...
        const home = dshHome();
        const fiber = injectorCtx.plugin(injector, {
