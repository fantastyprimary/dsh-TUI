...
export function createChannel(ctx, initialAgent, options) {
    ...
    const state = {
        ...
        async resumeTo(sessionId) {
            ...
            const resumeComposed = await composePreset(ctx, await resolvePersistedPreset(ctx, SessionId(sessionId)), resumedSmart);
        ...
        async switchSmart(enabled = !state.smart) {
            ...
            try {
                ...
                state.notify(t('smart-switch-failed', { err: error instanceof Error ? error.message : String(error) }), { color: 'error', timeoutMs: 8000 });
                return false;
            }
            const childId = SessionId(randomUUID());
            ...
            try {
                ...
                state.notify(`Smart switched, but workspace attachment failed · ${error instanceof Error ? error.message : String(error)}`, { color: 'warning', timeoutMs: 8000 });
            }
            streaming = undefined;
            reasoning = undefined;
            toolCards.clear();
