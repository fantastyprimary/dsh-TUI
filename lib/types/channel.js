...
import { readForceSmartDefault, resolvePersistedForceSmart, writeForceSmartDefault, writeForceSmartSession, } from './forceSmartPrefs.js';
...
export function createChannel(ctx, initialAgent, options) {
    ...
    const switchEnhancement = async (kind, enabled) => {
        ...
        if (enabled === current) {
            state.notify(t(`${key}-already`, { state: enabled ? 'on' : 'off' }), { color: 'success' });
            return true;
        }
        const sessions = ctx.get('sessions');
        const agents = ctx.get('agents');
        if (!sessions || !agents) {
            state.notify(t(`${key}-unavailable`), { color: 'error' });
            return false;
        }
        let seed;
        try {
            ...
            state.notify(t(`${key}-switch-failed`, { err: error instanceof Error ? error.message : String(error) }), { color: 'error', timeoutMs: 8000 });
            return false;
        }
        const targetSmart = kind === 'smart' ? enabled : enabled ? false : state.smart;
        const targetForceSmart = kind === 'force-smart' ? enabled : enabled ? false : state.forceSmart;
        const childId = SessionId(randomUUID());
        const composed = await composePreset(ctx, runningPresetOf(agent.session), targetSmart, targetForceSmart);
        let replacement;
        try {
        ...
        try {
            ...
            state.notify(`${displayName} switched, but workspace attachment failed · ${error instanceof Error ? error.message : String(error)}`, { color: 'warning', timeoutMs: 8000 });
        }
        streaming = undefined;
        reasoning = undefined;
        toolCards.clear();
    ...
    const state = {
        ...
        forceSmart: options.forceSmart === true,
        goal: undefined,
        todos: [],
        ...
        async rewindTo(row) {
            ...
            const rewindComposed = await composePreset(ctx, runningPresetOf(agent.session), state.smart, state.forceSmart);
            try {
        ...
        async resumeTo(sessionId) {
            ...
            const resumedForceSmart = await resolvePersistedForceSmart(ctx, SessionId(sessionId));
            const resumedSmart = resumedForceSmart
                ? false
                : await resolvePersistedSmart(ctx, SessionId(sessionId));
            const resumeComposed = await composePreset(ctx, await resolvePersistedPreset(ctx, SessionId(sessionId)), resumedSmart, resumedForceSmart);
        ...
        async newSession() {
            ...
            const { smart: newSmart, forceSmart: newForceSmart } = resolveEnhancementSelection(options.configuredSmart, options.configuredForceSmart, readSmartDefault(), readForceSmartDefault());
            const newComposed = await composePreset(ctx, options.configuredPreset ?? readPresetPref(), newSmart, newForceSmart);
        ...
        async switchModel(provider, model) {
            ...
            const modelComposed = await composePreset(ctx, runningPresetOf(agent.session), state.smart, state.forceSmart);
            try {
                handle = await agents.create({
                    sessionId: childId,
                    seed,
        ...
        async switchSmart(enabled = !state.smart) {
            return await switchEnhancement('smart', enabled);
        },
        async switchForceSmart(enabled = !state.forceSmart) {
            return await switchEnhancement('force-smart', enabled);
        },
        listEfforts,
        setEffort,
        cycleMode,
