 ...
 *
 * "在屏"判定：目标文本必须出现在浮层上下分隔线之间，并带焦点标记 `❯`；
 * 转录里同文本的用户消息位于浮层之外，不会误判为在屏。
 *
...
/**
 * 焦点行是否在屏：浮层位于输入框上方，由最后三条横线中的前两条界定。
 * 只检查该区域内带 `❯` 的目标行，避免 NO_COLOR/终端色深影响 RGB 断言。
 */
function focusLineVisible(text: string): boolean {
  const buf = term.buffer.active
  const rows = Array.from({ length: ROWS }, (_, index) => ({
    y: buf.baseY + index,
    text: buf.getLine(buf.baseY + index)?.translateToString(true) ?? '',
  }))
  const separators = rows.filter(row => /^\s*─{10}/u.test(row.text)).map(row => row.y)
  const overlayStart = separators.at(-3) ?? buf.baseY
  const overlayEnd = separators.at(-2) ?? (buf.baseY + ROWS)
  for (const row of rows) {
    if (row.y <= overlayStart || row.y >= overlayEnd) continue
    if (row.text.includes(text) && /^\s*❯\s/u.test(row.text)) return true
  }
  return false
}
